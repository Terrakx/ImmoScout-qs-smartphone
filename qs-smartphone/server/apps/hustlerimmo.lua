...
QBCore.Functions.CreateCallback('phone:server:getHouses', function(source, cb)
    local src = source
    ...
    MySQL.query('SELECT owned, name, label, price, coords, tier FROM `houselocations`', {}, function(result)
        ...
        for i=1, #result do
            ...
            elseif result[i].owned then
                local result2 = MySQL.query.await('SELECT house FROM `player_houses` WHERE citizenid=? AND house=?', {Player.PlayerData.citizenid, result[i].name})
